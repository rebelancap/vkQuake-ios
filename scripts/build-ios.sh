...
# app icon: legacy CFBundleIconFiles PNGs (Xcode 26 actool rejects single-size
# asset catalogs — q2repro-ios verified fact). Sizes generated from the master.
ICON_SRC="$IOSDIR/icon/vkq_icon.png"; ICON_GEN="$IOSDIR/icon/generated"
if [ -f "$ICON_SRC" ]; then
	mkdir -p "$ICON_GEN"
	sips -z 120 120 "$ICON_SRC" --out "$ICON_GEN/AppIcon60x60@2x.png" >/dev/null 2>&1
	sips -z 180 180 "$ICON_SRC" --out "$ICON_GEN/AppIcon60x60@3x.png" >/dev/null 2>&1
	sips -z 152 152 "$ICON_SRC" --out "$ICON_GEN/AppIcon76x76@2x~ipad.png" >/dev/null 2>&1
	sips -z 167 167 "$ICON_SRC" --out "$ICON_GEN/AppIcon83.5x83.5@2x~ipad.png" >/dev/null 2>&1
	echo "== icon generated from $ICON_SRC"
fi
